...
turtle_t enlarge_array(turtle_t *tg, int increase)
{
  ...
  temp = (turtle_t*)calloc(increase, sizeof(turtle_t));
...
void turtle_prog(FILE *turtle)
{
   ...
   while(y < MAX_INPUT){
    tg->wd[y] = (char*)calloc(MAX_LENGTH, sizeof(char));
    y++;
   }
   *tg = initialise_struct(tg);

   while(fscanf(turtle, "%s", str) == 1){
     strcpy(tg->wd[tg->now], str);
     tg->now++;
     ++cnt;
...
void instruction_statement(turtle_t *tg, SDL_t *start, Loop_t *dl)
{
  ...
  /*Set has also been included in this function as i believe its purpose
...
of the do loop onto a stack, such as the position within the text file, current iteration of
